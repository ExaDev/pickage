...
export interface PackageAutocompleteInputProps {
  ...
  /** Called when user submits (Enter, blur, or autocomplete selection) */
...
>(
  (
    { index, value, searchQuery, onChange, onSubmit, onRemove, showRemove },
    ref,
  ) => {
    ...
    return (
      <Group gap="md">
        <Autocomplete
          ...
          onKeyDown={(event) => {
            if (event.key === "Enter") {
              // User pressed Enter - submit current value
              event.preventDefault();
              onSubmit(value);
            }
          }}
          onBlur={() => {
            // User clicked/tabbed away - submit current value
            if (value.trim()) {
              onSubmit(value);
            }
          }}
